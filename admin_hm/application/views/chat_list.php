...
<div class="row">
	<div class="col-lg-12">
		<h3 class="page-header">聊天记录 <button class="btn btn-danger" onclick="clear_redis();">清除Redis</button><button class="btn btn-danger" onclick="clear_mysql();">清除MYSQL</button></h3>
	</div>
	<!-- /.col-lg-12 -->
</div>
...
function clear_redis(){
	// if(confirm("确认清除Redis数据库中的所有消息记录")){
		$.get(admin.url+'chat/clear_redis','',function (){alert("删除成功");});
	// }
}

function clear_mysql(){
	if(confirm("清除MYSQL数据库后，将不能再查找聊天记录，请慎重！")){
		$.get(admin.url+'chat/clear_mysql','',function (){alert("删除成功");});
	}
}
function for_excel(){
