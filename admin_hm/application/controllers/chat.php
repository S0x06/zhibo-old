...
class chat extends MY_Controller {
	...
	function search()
	{
		...
		if( ! empty($send) )
		{
			$search_send = "'".str_replace(',', "','", $send)."'";
			$search .= " AND name IN ({$search_send}) ";
		}
		if( ! empty($start) ) $search .= " AND time >= '{$start}' ";
		if( ! empty($end) ) $search .= " AND time <= '{$end}' ";
		$types = " AND types = 1 ";
		if( $rid !== 0 AND ! empty($rid) ) $search .= " AND rid = '{$rid}' ";

		$limit = 30;
		$start_record = ($page - 1)*$limit;
		$tb_chat_list = $this->db->dbprefix('chat_list');
		$this->out_data['record_list'] = $this->db->query("select id,name,time,content from {$tb_chat_list} where 1= 1 {$search} order by time desc limit {$start_record},{$limit}")->result_array();

		$this->out_data['record_count'] = $this->db->query("select count(1) as num from {$tb_chat_list} where 1 = 1 {$search}")->row()->num;
		$this->out_data['ben_count'] = $this->db->query("select count(1) as num from {$tb_chat_list} where 1 = 1 {$search}{$types}")->row()->num;

		$this->out_data['pagin'] = parent::get_pagin(base_url()."chat/search?send={$send}&start={$start}&end={$end}&rid={$rid}", $this->out_data['record_count'], $limit, 3,  true);
