...
class specialist extends MY_Controller {
	...
	function specialist_list($room_id = '')
	{
		...
		$this->out_data['specialist_list'] = $this->db->query("select id,name from {$tb_specialist} where rid='{$room_id}'")->result_array();
	...
	{
		$info = $this->db->query("select id,name,rid,content,avatar from {$this->db->dbprefix('specialist')} where id={$id} limit 1");
		if($info->num_rows() > 0)
		{
			$info = $info->row_array();
		}
